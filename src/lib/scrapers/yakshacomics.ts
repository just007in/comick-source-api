...
export class YakshacomicsScraper extends BaseScraper {
  private readonly BASE_URL = "https://yakshacomics.com";

  getName(): string {
    return "Yaksha Comics";
  }
  ...
  canHandle(url: string): boolean {
    return url.includes("yakshacomics.com");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      console.error("[Yakshacomics] Chapter fetch error:", error);
      throw error;
    }
    ...
    const results = await mapWithConcurrencyLimit(
      ...
      async (series): Promise<SearchResult> => {
        try {
          ...
          console.error(
            `[Yakshacomics] Failed to fetch chapter list for ${series.title}:`,
            error
          );
          return {
            id: series.id,
