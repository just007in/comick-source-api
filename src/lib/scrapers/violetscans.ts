...
export class VioletscansScraper extends BaseScraper {
  private readonly BASE_URL = "https://violetscans.org";
  ...
  canHandle(url: string): boolean {
    return url.includes("violetscans.org");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      console.error("[Violetscans] Chapter fetch error:", error);
      throw error;
    }
    ...
    const results = await mapWithConcurrencyLimit(
      ...
      async (series): Promise<SearchResult> => {
        try {
          ...
          console.error(
            `[Violetscans] Failed to fetch chapter list for ${series.title}:`,
            error
          );
          return {
            id: series.id,
