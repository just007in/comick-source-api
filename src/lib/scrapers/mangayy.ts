...
export class MangayyScraper extends BaseScraper {
  getName(): string {
    return "Mangayy";
  }
  ...
  canHandle(url: string): boolean {
    return url.includes("mangayy.org");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      try {
        ...
        console.log("[Mangayy] AJAX failed, falling back to regular page");
      ...
      console.error("[Mangayy] Chapter fetch error:", error);
    }
  ...
  async search(query: string): Promise<SearchResult[]> {
    const searchUrl = `https://mangayy.org/?s=${encodeURIComponent(query)}&post_type=wp-manga`;
    ...
    $(".page-item-detail").each((_, element) => {
      ...
      const coverImg = $item.find(".item-thumb img").first();
      ...
      const lastUpdatedSpan = $item
        .find(".list-chapter .chapter-item .post-on")
        .first();
      const lastUpdated = lastUpdatedSpan.text().trim();

      const ratingDiv = $item.find(".meta-item.rating .score").first();
