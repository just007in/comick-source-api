...
export class MangasushiScraper extends BaseScraper {
  private readonly BASE_URL = "https://mangasushi.org";
  ...
  canHandle(url: string): boolean {
    return url.includes("mangasushi.org");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      console.error("[Mangasushi] Chapter fetch error:", error);
      throw error;
    }
    ...
    const results = await mapWithConcurrencyLimit(
      ...
      async (series): Promise<SearchResult> => {
        try {
          ...
          console.error(
            `[Mangasushi] Failed to fetch chapter list for ${series.title}:`,
            error
          );
          return {
            id: series.id,
