...
export class YakshascansScraper extends BaseScraper {
  private readonly BASE_URL = "https://yakshascans.com";
  ...
  canHandle(url: string): boolean {
    return url.includes("yakshascans.com");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      console.error("[Yakshascans] Chapter fetch error:", error);
      throw error;
    }
    ...
    const results = await mapWithConcurrencyLimit(
      ...
      async (series): Promise<SearchResult> => {
        try {
          ...
          console.error(
            `[Yakshascans] Failed to fetch chapter list for ${series.title}:`,
            error
          );
          return {
            id: series.id,
