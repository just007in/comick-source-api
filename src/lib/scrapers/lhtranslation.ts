...
export class LHTranslationScraper extends BaseScraper {
  private readonly BASE_URL = "https://lhtranslation.net";
  ...
  canHandle(url: string): boolean {
    return url.includes("lhtranslation.net");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      console.error("[LHTranslation] Chapter fetch error:", error);
      throw error;
    }
    ...
    const results = await mapWithConcurrencyLimit(
      ...
      async (series): Promise<SearchResult> => {
        try {
          ...
          console.error(
            `[LHTranslation] Failed to fetch chapter list for ${series.title}:`,
            error
          );
          return {
            id: series.id,
