...
export class FirescansScraper extends BaseScraper {
  private readonly BASE_URL = "https://firescans.xyz";
  ...
  canHandle(url: string): boolean {
    return url.includes("firescans.xyz");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      console.error("[Firescans] Chapter fetch error:", error);
      throw error;
    }
    ...
    const results = await mapWithConcurrencyLimit(
      ...
      async (series): Promise<SearchResult> => {
        try {
          ...
          console.error(
            `[Firescans] Failed to fetch chapter list for ${series.title}:`,
            error
          );
          return {
            id: series.id,
