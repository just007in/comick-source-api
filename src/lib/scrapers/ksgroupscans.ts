...
export class KsgroupscansScraper extends BaseScraper {
  private readonly BASE_URL = "https://ksgroupscans.com";
  ...
  canHandle(url: string): boolean {
    return url.includes("ksgroupscans.com");
  }
  ...
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    try {
      ...
      console.error("[Ksgroupscans] Chapter fetch error:", error);
      throw error;
    }
    ...
    const results = await mapWithConcurrencyLimit(
      ...
      async (series): Promise<SearchResult> => {
        try {
          ...
          console.error(
            `[Ksgroupscans] Failed to fetch chapter list for ${series.title}:`,
            error
          );
          return {
            id: series.id,
